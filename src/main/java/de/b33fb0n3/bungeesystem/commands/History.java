...
public class History extends Command {
    ...
    private void sendHelp(int seite, ProxiedPlayer pp, UUID uuid) {
        ...
        int zeilen = 10;
        int allReports = DBUtil.getWhatCount(Bungeesystem.getPlugin().getDataSource(),uuid, "", false);
        ...
        for(HistoryElemt history : histroy) {
            ...
            int i = 1;
            Ban ban = new Ban(history.getTargetUUID(), null, Bungeesystem.getPlugin().getDataSource(), Bungeesystem.settings, Bungeesystem.standardBans);
            while (true) {
                try {
                    String line = ChatColor.translateAlternateColorCodes('&', Bungeesystem.settings.getString("History.hover." + i)).replace("%von%", UUIDFetcher.getName(history.getVonUUID())).replace("%bis%", (history.getBis() == -2 ? "Permanent" : Bungeesystem.formatTime(history.getBis()))).replace("%zeit%", new Time(history.getErstellt()).toString()).replace("%status%", (history.getBan() != -2 ? (history.getBan() == 0 ? "Mute" : "Ban") : "Keiner")).replace("%type%", typeForm).replace("%grund%", history.getGrund()).replace("%time%", simpleDateFormat.format(date)).replace("%aktiv%", type.equalsIgnoreCase("ban") ? (ban.isBanned() ? "§a✔" : "§c✖") : "§a✔").replace("%entbanner%", history.getVonEntbannt() == null ? "Keiner" : history.getVonEntbannt());
                    hoverArray.add(line);
                    i++;
                    if (i > 6)
                        break;
