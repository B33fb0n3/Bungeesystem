...
public class Bans extends Command {
    ...
    public void execute(CommandSender sender, String[] args) {
        if (sender instanceof ProxiedPlayer) {
            ...
            if (pp.hasPermission("bungeecord.bans") || pp.hasPermission("bungeecord.*")) {
                if (args.length == 2 || args.length == 1) { // /bans <Spieler> [<Seite>]
                    int seite = 1;
                    try {
                    ...
                    try {
                        ...
                        pp.sendMessage(new TextComponent(Bungeesystem.Prefix + Bungeesystem.fehler + "Dieser Spieler existiert nicht!"));
                    }
                } else
                    pp.sendMessage(new TextComponent(Bungeesystem.helpMessage.replace("%begriff%", "bans")));
            } else
